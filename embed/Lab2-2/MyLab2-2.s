...
;  /*****************************************************
				...
				LDR	R1, =0			; r1 = 0							r1에 0을 넣어준다.
				LDR	R0, =0			; i = 0							for문을 위해 r0를 i 로 사용한다.
label ;																		label 지정
				ADD R1, R1, R0	; r1 += i							r1 += i를 실행한다.
				ADD R0, R0, #1		; i++								i++을 실행한다.
				CMP R0, #10		; EQ = i == 10?				i와 10을 비교연산한다.
				BNE label				; branch if NE to label	i != 10 이면 label로 이동한다.
